...
export default {
  ...
  effects: {
    *fetch({ queryPara, callback, exportFlag }, { call, put }) {
      ...
      const response = yield call(services.get, '/api/ss/lt-student-infos', newQueryPara);
      if (response) {
    ...
    *add({ payload, callback }, { call, put }) {
      const response = yield call(services.post, '/api/ss/lt-student-infos', payload);
      if (response.id) {
        yield put({
          type: 'addsave',
          payload: response,
    ...
    // 批量提交绑定到考试中
    *batchBinding({ payload, callback }, { call }) {
      const response = yield call(
        services.put,
        '/api/ss/lt-exam-students/add-student-exam',
        payload
      );
      if (response) {
        if (callback) callback(response);
    ...
    *allBinding({ payload, queryPara, callback }, { call }) {
      const response = yield call(
        services.get,
        `/api/lt-exam-students/add-list-student/${payload.examCode}`,
        queryPara
      );
      if (response) {
        if (callback) callback(response);
    ...
    *adds({ payload, callback }, { call, put }) {
      const response = yield call(services.post, '/api/lt-student-infos-batch', payload); // 后台接口地址
      if (response.length) {
        yield put({
          type: 'addssave',
          payload: response,
    ...
    *update({ payload, callback }, { call, put }) {
      const response = yield call(services.put, '/api/ss/lt-student-infos', payload);
      if (response.id) {
        yield put({
          type: 'addsave',
          payload: response,
    ...
    *remove({ payload, callback }, { call }) {
      const response = yield call(services.del, '/api/lt-student-infos', payload);
      if (response) {
        if (callback) callback(response);
      }
    },
