...
export default {
  ...
  effects: {
    *fetchBingStudent({ queryPara, callback, exportFlag }, { call, put }) {
      const response = yield call(
        services.get,
        `/api/ss/lt-exam-students/students-exam/${queryPara.examCode}`,
        queryPara
      );
      if (response) {
    ...
    *add({ payload, callback }, { call, put }) {
      const response = yield call(services.post, '/api/lt-train-enrolls', payload);
      if (response.id) {
        yield put({
          type: 'addsave',
          payload: response,
    ...
    *adds({ payload, callback }, { call, put }) {
      const response = yield call(services.post, '/api/lt-train-enrolls-batch', payload); // 后台接口地址
      if (response.length) {
        yield put({
          type: 'addssave',
          payload: response,
    ...
    // 从考试中移除
    *batchRemoveBinding({ payload, callback }, { call }) {
      const response = yield call(
        services.post,
        '/api/ss/lt-exam-students/remove-student-exam',
        payload
      );
      if (response) {
        if (callback) callback(response);
    ...
    *update({ payload, callback }, { call, put }) {
      const response = yield call(services.put, '/api/lt-train-enrolls', payload);
      if (response.id) {
        yield put({
          type: 'addsave',
          payload: response,
    ...
    *remove({ payload, callback }, { call }) {
      const response = yield call(services.del, '/api/lt-train-enrolls', payload);
      if (response) {
        if (callback) callback(response);
      }
    },
