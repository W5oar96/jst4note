...
//        searchSourceBuilder.sort("publishTime", SortOrder.DESC);
    ...
    public PubRespInfoModel searchNew(QueryForm queryForm, List<String> types) throws IOException {
        ...
        if(queryForm.getQueryStringList() != null){
        ...
        highlightBuilder.preTags(HIGHLIGHT_PRE_TAGS);
        highlightBuilder.postTags(HIGHLIGHT_POST_TAGS);
        if(queryForm.getHighlightFieldList() != null){
        ...
        searchSourceBuilder.from((queryForm.getPageNum() == null ? 0 :queryForm.getPageNum() - 1) * (queryForm.getPageSize() == null ? 10 : queryForm.getPageSize()));
        searchSourceBuilder.size(queryForm.getPageSize() == null ? 10 : queryForm.getPageSize());
        ...
        try {
            ...
            if (result != null && result.isSucceeded()) {
                ...
                data = handlerDataNew(data, queryForm.getHighlightFieldList(),result);
                SearchVO vo = new SearchVO();
                if(result.getTotal() != null){
                    vo.setTotal(result.getTotal().longValue());
                }else{
                    vo.setTotal(0L);
                }
                if(queryForm.getPageNum() != null){
                    vo.setPageNum(queryForm.getPageNum());
                }else{
                    vo.setPageNum(1);
                }
                ...
                if(queryForm.getQueryStringList() != null){
                    ...
                    infoModel.setMsg("搜索 <span class='highlight'>" + searchData + "</span> 找到 " + vo.getTotal() + " 个与之相关的内容，耗时：" + (System.currentTimeMillis() - startTime) +"ms");
                }else{
                    infoModel.setMsg("搜索找到 " + vo.getTotal() + " 个与之相关的内容，耗时：" + (System.currentTimeMillis() - startTime) +"ms");
                }
                infoModel.setData(vo);
                infoModel.setFlg(PubRespInfoModel.FLG_SUCCESS);
            } else if (result != null){
                infoModel.setMsg(result.getErrorMessage());
                log.error("ES 查询失败：{}",result.getErrorMessage());
            }else{
                infoModel.setMsg("ES 查询失败：result为空");
                log.error("ES 查询失败：result为空");
            }
        } catch (IOException e) {
            log.error(e.getMessage());
            infoModel.setMsg(e.getMessage());
    ...
    // 新方法名：handlerDataNew (处理 Jest 返回的高亮信息)
    private List<Map<String, Object>> handlerDataNew(List<Map<String, Object>> data, List<String> fieldList, JestResult result) {
        ...
        if(result.getJsonObject() != null && result.getJsonObject().has("hits") && result.getJsonObject().getAsJsonObject("hits").has("hits") && result.getJsonObject().getAsJsonObject("hits").getAsJsonArray("hits") != null){
            highlights = result.getJsonObject().getAsJsonObject("hits").getAsJsonObject("hits").entrySet().stream().collect(Collectors.toMap(
                ...
                e -> {
                    if (e.getAsJsonObject().has("highlight")) {
                        return e.getAsJsonObject().getAsJsonObject("highlight").entrySet().stream().collect(Collectors.toMap(
                            e1 -> e1.getKey(),
                            e1 -> {
                                JsonArray jsonArray = e1.getValue().getAsJsonArray();
